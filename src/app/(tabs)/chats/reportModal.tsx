...
export default function ReportModal() {
	...
	return (
		<View style={tw`px-6 py-12 bg-white rounded-t-2xl`}>
			<TouchableOpacity
			...
			<Text style={tw`text-lg font-poppinsSemiBold w-full text-center`}>
				Why are you reporting this user?
			</Text>
			<Text style={tw`text-sm font-poppins w-full text-center mt-2`}>
				...
				{[
					...
					<TouchableOpacity
						...
						style={tw`w-full border border-gray-300 rounded-lg p-4 ${
							selectedReason === reason ? 'bg-gray-400' : 'bg-gray-100'
						}`}
						onPress={() => {
					...
					>
						<Text
							style={tw`text-sm font-poppins ${
								selectedReason === reason ? 'text-white' : 'text-black'
							}`}
						>
							{reason}
						</Text>
